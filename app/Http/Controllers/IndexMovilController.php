...
class IndexMovilController extends Controller
{
    ...
    public function index($sucursal){
        return view('index.movil.index',['sucursal'=>$sucursal]);
    }
    public function gracias($sucursal){
        return view('index.movil.gracias',['sucursal'=>$sucursal]);
    }
    public function options($sucursal,$nombre,$telefono){
        return view('index.movil.options',['sucursal'=>$sucursal,'nombre'=>$nombre,'telefono'=>$telefono]);
    }
    public function comentario($sucursal,$nombre,$telefono){
        $preguntas = Pregunta::where('estado_pregunta',1)->where('tipo_pregunta',6)->get();
        //dd($preguntas);
        return view('index.movil.comentario',['sucursal'=>$sucursal,'preguntas'=>$preguntas,'nombre'=>$nombre,'telefono'=>$telefono]);
    }
    public function encuesta($sucursal,$nombre,$telefono){
        $preguntas = Pregunta::where('estado_pregunta',1)->get();
        //dd($preguntas);
        return view('index.movil.encuesta',['sucursal'=>$sucursal,'preguntas'=>$preguntas,'nombre'=>$nombre,'telefono'=>$telefono]);
    }
